...
DEFAULT_OUTPUT_ROOT = SIMULATOR_DIR / "results" / "topology_inference" / "passive_ego_mesh"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the passive_ego_mesh prototype.")
    ...
    parser.add_argument(
        ...
        required=True,
        type=int,
        help="Attacker network index resolved internally to the real node id.",
    )
    parser.add_argument(
        "--late-join",
        action="store_true",
        help="Keep the observer dormant until --start-heartbeat, then let it join.",
    )
    parser.add_argument(
        "--start-heartbeat",
        type=int,
        default=20,
        help="Heartbeat at which a --late-join observer becomes active.",
    )
    parser.add_argument(
        "--late-join-topics",
        nargs="*",
        default=[],
        help=(
            "Optional topics to join immediately at activation. If omitted, mono-topic runs join "
...
def build_run_dir(
    *,
    ...
    return (output_root / run_label / f"seed_{seed}_attacker_{attacker_index}").resolve()
...
def build_simulator_command(
    *,
    ...
    cmd = [
        ...
        "gossipsub.inference.passive_ego_mesh.enabled=true",
        f"gossipsub.inference.passive_ego_mesh.attacker_index={attacker_index}",
        f"gossipsub.inference.passive_ego_mesh.scenario_name={scenario_name}",
    ]
    if late_join:
        cmd.extend(
            [
                "gossipsub.inference.passive_ego_mesh.late_join=true",
                f"gossipsub.inference.passive_ego_mesh.start_heartbeat={start_heartbeat}",
            ]
        )
        topics = [topic.strip() for topic in (late_join_topics or []) if topic.strip()]
        if topics:
            cmd.append(
                "gossipsub.inference.passive_ego_mesh.join_topics=" + ",".join(topics)
            )
    return cmd
